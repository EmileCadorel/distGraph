module utils.syntax.Lexer;
public import utils.syntax.Word;
public import utils.syntax.Tokens;
...
class SyntaxError : Exception {
	...
	buf.writef ("Erreur de syntaxe(%d, %d) : %s, Attendue : [", word.locus.line, word.locus.column, word.str);
	foreach (it ; expected) {
	    buf.writef ("%s", it);
	}	
	buf.writefln ("] : ");
	buf.writefln (word.locus.mixLines);
	foreach (it ; 1 .. word.locus.column)
	    buf.write (" ");
	
	foreach (it ; 0 .. word.locus.length)
	    buf.write ("^");	
...
class StringLexer : Lexer {
    ...
    this (string content, Token [] skips) {
	Token [2][] comments;
